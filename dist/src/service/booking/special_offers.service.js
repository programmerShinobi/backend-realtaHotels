...
let SpecialOffersService = class SpecialOffersService {
    ...
    async findGuestSpecialOffers() {
        const today = new Date();
        const specialOffers = await this.specialOffersRepository
            .createQueryBuilder('so')
            .leftJoin('(SELECT soco.soco_spof_id, COUNT(soco.soco_spof_id) AS socoCount FROM booking.special_offer_coupons soco GROUP BY soco.soco_spof_id)', 'soco', 'so.spofId = soco.soco_spof_id')
